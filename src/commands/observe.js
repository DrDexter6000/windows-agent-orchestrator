...
const COLLECT_PROJECTION_KNOWN_FLAGS = new Set([
  "cursor", "format", "limit", "runDir", "cwd",
]);
...
export async function collectCommand(args, config) {
  ...
  const hasCursor = options.cursor !== undefined && options.cursor !== true;
  ...
  // M11-4 §12: defer audit append until projection validates the cursor.
  const hasCursorForDefer = cursor !== null;
  const raw = await collectRunMessages({ runId, runDir, cursor, deferAppend: hasCursorForDefer });
  const payload = projectCollectResult(raw, { runId, cursor });
  // Projection succeeded → safe to commit the audit (cursor mode only).
