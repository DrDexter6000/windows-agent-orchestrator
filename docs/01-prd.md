...
- **Skill-guided**：`SKILL.md` 负责提供成熟的工程思维、角色合同与可选工作流模板，告诉 Lead 何时派工、如何验收和遵守边界。Skill 不保存运行状态，不代替 transcript，不实现控制逻辑。Lead 继续负责语义分解、失败响应和最终验收。WAO 不自动执行语义任务分解。
...
| MCP Server（agent-facing primary 控制面） | [M9] | ✅ 当前 **11 tools**（registry_list/workspace_status/run_dispatch/run_status/run_collect/run_diagnose/run_delivery/run_delivery_decide/run_stop/runs_list/run_wait；Codex + Claude Code/Fable 两 runtime dogfood + M10 真实外部项目多 worker dogfood PASS）。schema 见 `docs/usage.md` / `docs/02-architecture.md` |
| 事件订阅（通用化 tail --follow） | [M] | 🟡 已有文件轮询式 |
| shared application services（MCP 与 CLI 共用，禁止 shell-out） | [M9] | ✅ M9/M10 最小 Lead 闭环 services 已完成提取（registryInventory/runDispatch/runStatus/runCollect/runDiagnosis/runDelivery/runStop/runList/runWait/ownerLiveness/workspaceBinding 等）；非闭环 use-case 仍可能在 commands |
...
| 能力 | 阶段 | 现状 |
|------|------|------|
| metrics 聚合（token / 延迟 / 成功率） | [S→M] | ✅ M4 单 run 聚合 + M8 dashboard 跨 run 聚合已落地 |
| scorecard 证据链门控（程序审计，非 LLM 自报） | [M] | ✅ M6 scorecard + M8-1 默认 warn 已落地 |
| 验收契约机制（无对照物场景） | [L] | ✅ M7-P1 用户验收脚本 + TD-103 delivery verification + Lead acceptance 已落地 |
...
7. **实际跑通**：在真实外部项目上完成至少一次真实的多 agent 并行开发任务（M10 已由 Smash Bros 真实多 worker dogfood 完成产品级验收；Smash Bros delivery 仅在 WAO 侧验证，未集成进目标项目）。
