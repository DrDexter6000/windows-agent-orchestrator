...
> **本节定义能力范围，不维护实现进度。** 当前完成状态只见 `docs/roadmap.md`；接口契约、tool inventory 与 application-service 清单见 `docs/02-architecture.md` 与 `docs/usage.md`。本节不再复制 tool 枚举或 service 文件清单。

### L1 运行时抽象

| 能力 | 阶段 |
|------|------|
| Backend 接口（spawn / messages / abort / healthCheck / waitForCompletion） | [S] |
| 统一消息流 `AsyncIterable<RunEvent>`（屏蔽 HTTP 轮询 / stdio 流式差异） | [S] |
| 进程式 backend（claude-code / codex / kimi-code，走 stdio） | [S] |
| Registry 加载与规范化 | [S] |
| Transcript（JSONL，事件追加） | [S] |

### L2 控制平面

| 能力 | 阶段 |
|------|------|
| 显式状态机（见 `02-architecture.md §状态机`） | [S] |
| run 恢复（runs resume，基于 transcript 重接） | [S] |
| 优雅关闭（Ctrl+C abort 活跃会话） | [S] |
| 多 run 并行 + 限并发 | [S] |
| worktree 隔离 + 进程组 + 端口表 | [S] |
| 持久 daemon + IPC | [M] |
...
| 能力 | 阶段 |
|------|------|
| CLI（human/ops/debug/fallback 接口） | [S] |
| MCP Server（agent-facing primary 控制面） | [M9] |
...
7. **实际跑通**：在真实外部项目上完成至少一次真实的多 agent 并行开发任务（完成事实与具体 runId/commit 见 `docs/roadmap.md` M10 行；delivery 仅在 WAO 侧验证，未集成进目标项目）。
