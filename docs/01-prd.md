...
| MCP Server（agent-facing primary 控制面） | [M9] | 🔄 in progress（M9-1~5 registry_list/run_dispatch/run_status/run_collect/run_diagnose 已落地；delivery query/acceptance 尚未在 MCP 实现） |
| 事件订阅（通用化 tail --follow） | [M] | 🟡 已有文件轮询式 |
| shared application services（MCP 与 CLI 共用，禁止 shell-out） | [M9] | 🔄 in progress（registryInventory + runDispatch + runStatus + runCollect + runDiagnosis 已提取；其余 use-case 待提取） |
