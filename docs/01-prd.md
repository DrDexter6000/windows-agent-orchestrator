...
| **次要** | 想要可脚本化/CI 驱动的 agent 编排、且排斥重 GUI 的 power user |
...
- **U5 等价驱动**：bash 脚本、未来的 LLM 编排器、我自己敲 CLI，三者在同一套原语上**平权**。
