...
| MCP Server（agent-facing primary 控制面） | [M9] | 🔄 in progress（M9-1~6 registry_list/run_dispatch/run_status/run_collect/run_diagnose/run_delivery/run_delivery_decide 已落地；M9 仍需真实 MCP dogfood + Skill/SSOT 关单） |
| 事件订阅（通用化 tail --follow） | [M] | 🟡 已有文件轮询式 |
| shared application services（MCP 与 CLI 共用，禁止 shell-out） | [M9] | 🔄 in progress（registryInventory + runDispatch + runStatus + runCollect + runDiagnosis + runDelivery 已提取；其余 use-case 待提取） |
