...
| MCP Server（agent-facing primary 控制面） | [M9] | 🔄 in progress（M9-1 registry_list + M9-2B run_dispatch + M9-3B run_status + M9-4B run_collect 已落地；diagnose/delivery/acceptance 尚未在 MCP 实现） |
| 事件订阅（通用化 tail --follow） | [M] | 🟡 已有文件轮询式 |
| shared application services（MCP 与 CLI 共用，禁止 shell-out） | [M9] | 🔄 in progress（M9-0 registryInventory + M9-2A runDispatch + M9-3A runStatus + M9-4A runCollect 已提取并供 CLI/MCP 共用；其余 use-case 待提取） |
