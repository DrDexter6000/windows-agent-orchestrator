...
| M9 | 🔄 in progress | Agent Runtime Control Surface：MCP Server 作为 agent-facing primary 控制面，与 CLI 共享 application services。Decision 0017 定义方向。**M9-0 已完成**：`registryInventory.js`（inventory）。**M9-1 已完成**：只读 MCP stdio 垂直切片（`registry_list`）。**M9-2A/B 已完成**：`runDispatch.js` + MCP `run_dispatch`（destructive，requireCertified:true）。**M9-3A/B 已完成**：`runStatus.js` + MCP `run_status`（只读，安全机器字段）。**M9-4A/B 已完成**：`runCollect.js` + MCP `run_collect`（有界安全投影：assistant 文本 + evidenceCounts，每次成功追加 messages.collected，非只读非幂等）。inventory + dispatch + status + collect 已完成；diagnose/delivery query/acceptance 尚未在 MCP 实现。完成门：从 command modules 提取最小 Lead 闭环 application services（CLI 委托，行为不变）、MCP/CLI 共用 services、最小 Lead 闭环、等价 state-changing operation 产生相同 transcript durable facts、不 shell-out CLI、真实 MCP dogfood（≥2 不同 Agent Runtime 作 Lead host）、CLI fallback 可用、TD-104 强隔离不放宽。 |
