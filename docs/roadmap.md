...
| M9 | 🔄 in progress | Agent Runtime Control Surface：MCP Server 作为 agent-facing primary 控制面，与 CLI 共享 application services。Decision 0017 定义方向。**M9-0 已完成**：`src/application/registryInventory.js` 提取为共享 application service，CLI `registry list` 已委托，text/JSON 输出契约不变。**M9-1 已完成**：最小只读 MCP stdio 垂直切片——`src/mcp/server.js`（factory + `registry_list` tool）+ `src/mcp/stdio.js`（`StdioServerTransport` entrypoint）+ `npm run mcp`，tool 直接复用 `getRegistryInventory()`，不 shell-out CLI，零 transcript 副作用，依赖边界守卫（仅 `src/mcp/**` 可 import MCP SDK/Zod）。MCP 尚未实现 Lead 闭环（dispatch/supervise/delivery/acceptance）。完成门：从 command modules 提取最小 Lead 闭环 application services（CLI 委托，行为不变）、MCP/CLI 共用 services、最小 Lead 闭环（inventory → dispatch → supervise → collect/diagnose → delivery query → acceptance）、等价 state-changing operation 产生相同 transcript durable facts、不 shell-out CLI、真实 MCP dogfood（≥2 不同 Agent Runtime 作 Lead host）、CLI fallback 可用、TD-104 强隔离不放宽。 |
