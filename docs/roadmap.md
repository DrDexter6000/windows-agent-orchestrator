...
关闭的生产摩擦：从 `C:\Users\<you>` 执行 `npm run cli -- runs dashboard --web --cwd ...` 报 "Missing script: cli"（npm 从调用方 cwd 解析脚本）。实现事实（TDD，非成功声明；真实 npm-link/browser canary 待 Lead 验收后另行记录）：
