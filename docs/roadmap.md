...
| M9 | 🔄 in progress | Agent Runtime Control Surface。**M9-0~5 已完成**：registry_list/inventory、run_dispatch/dispatch、run_status/status、run_collect/collect、run_diagnose/diagnose（含 provider_disconnect 优先级修复）。**M9-6A/B 已完成**：`runDelivery.js` + MCP `run_delivery`（只读查询）+ `run_delivery_decide`（destructive 持久决策）。**M9-7A 已完成**：`run_dispatch` 现支持 delivery-capable dispatch（可选 `delivery` 参数创建 git_commit_v1 delivery run，强制 persistent worktree 隔离 + 打包 + 验证）；`.dev/m97a-stdio-smoke.mjs` 以真实 stdio + 无模型 fake worker 跑通完整 delivery 闭环（dispatch→poll→delivery query→accept/reject loser）并验证 source 不变、heartbeat 清理、transcript 无泄漏。最小 Lead 工具闭环（inventory→dispatch→status→collect/diagnose→delivery query→acceptance）在代码层面已齐。M9 仍需：≥2 不同 Agent Runtime 作 Lead 的真实 MCP dogfood + 最终 Skill/SSOT 关单。完成门不变。 |
