...
- 验证：`test/ownerDashboardLauncher.test.js` TDD `14/14`（bin 声明/转发、严格默认、嵌套 root、非 Git 先拒、带空格 `--cwd`、恰好一次打开、`--no-open` 零尝试、打开失败 advisory 保活、遗留不打开+fail-soft、help 文档化、无 shell 生产代码）；focused 三文件 `100/100`（canonical env）与 dashboard 面 `80/80`；`git diff --check` clean。
