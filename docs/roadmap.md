...
| M9 | 🔄 in progress | Agent Runtime Control Surface：MCP Server 作为 agent-facing primary 控制面，与 CLI 共享 application services。Decision 0017 定义方向。**M9-0 已完成**：`src/application/registryInventory.js` 共享 service（inventory）。**M9-1 已完成**：只读 MCP stdio 垂直切片（`registry_list`）。**M9-2A 已完成**：`src/application/runDispatch.js` 共享后台派发 service，CLI `run --background`/`spawn`(no --wait) 委托，`requireCertified` 在后台路径不再静默忽略。**M9-2B 已完成**：MCP `run_dispatch` tool（strict input agentId+prompt，固定 requireCertified:true，输出仅 runId/accepted/state，detached runner 独立驱动到终态）。inventory + dispatch 已完成；supervise/collect/diagnose/delivery query/acceptance 尚未在 MCP 实现（M9-3+）。完成门：从 command modules 提取最小 Lead 闭环 application services（CLI 委托，行为不变）、MCP/CLI 共用 services、最小 Lead 闭环（inventory → dispatch → supervise → collect/diagnose → delivery query → acceptance）、等价 state-changing operation 产生相同 transcript durable facts、不 shell-out CLI、真实 MCP dogfood（≥2 不同 Agent Runtime 作 Lead host）、CLI fallback 可用、TD-104 强隔离不放宽。 |
