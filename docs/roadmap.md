...
| M11 | 📋 规划中（未开始） | **Lead Experience + Adaptive Playbooks**：两个核心——(1) 降低 Lead 使用摩擦，让常用闭环（派发/监督/collect/验收）更顺手；(2) 提供小而可选、可修改的 playbook/template，不强制统一 workflow，不自动做语义拆解。**完成门只定义价值，不在本任务实现**，至少包含：安全 artifact/changed-path review；bounded `run_collect` 截断时的 continuation/完整结果获取；Tester context/token efficiency；3~4 个可选工作流模板；Codex/Claude/Kimi Lead 可按需偏离模板；Advisor/Auditor 只在 Lead 低信心时可选调用。 |
