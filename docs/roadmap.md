...
| M9 | MCP Server 使用与 CLI 相同的 application services；最小 Lead 闭环可用（inventory → dispatch → supervise → collect/diagnose → delivery query → acceptance）；MCP/CLI 对同一操作产生相同 transcript 事实；不通过 shell 调 CLI；真实 MCP dogfood（至少两个不同 Agent Runtime 分别作为 Lead host 完成受监督任务）；CLI fallback 保持可用；TD-104 强隔离边界不因 MCP 接入而放宽 | MCP server 测试 + 跨 runtime dogfood |
...
| M9 | 🔲 planned（未开始） | Agent Runtime Control Surface：MCP Server 作为 agent-facing primary 控制面，与 CLI 共享 application services。Decision 0017 定义方向。完成门：MCP/CLI 共用 application services、最小 Lead 闭环（inventory → dispatch → supervise → collect/diagnose → delivery query → acceptance）、相同 transcript 事实、不 shell-out CLI、真实 MCP dogfood（≥2 不同 Agent Runtime 作 Lead host）、CLI fallback 可用、TD-104 强隔离不放宽。 |
