...
**M12-9 本地 Lead 验收与重集成事实（2026-08-02）**：真实 `coder_hq` 根 run `run_20260802162736427rmxxvj` 交付后，独立 `coder_mm` 风险审查提出 MCP 输出边界与文字归属问题；Lead 使用同一 provider conversation 和 retained worktree 显式续跑 child `run_2026080219443391071irlp`。child 首次封装因 continuation 候选相对 lineage 原始 base 的累计路径超出 correction-only allowed paths 而得到 `disallowed_path`；Lead 审查累计 24 个路径后明确批准同一 worktree 无模型 repackage，delivery `44917ef0f2d0c050a2fa7e0b98b783e80c20d376` exact verification passed。Lead 完整 review 后 durable accepted child，并 durable rejected 已被修正版取代的 root。该能力随后按 Lead 修正重集成到当前 main HEAD：保留 main 最新 Claude Code runtime 改动、重叠文件（`src/mcp/server.js`、docs、`mcpRunActivity.test.js`）按语义三方合并，未改 canonical runner 行为、package/config/runtime 凭据、worker registry 或无关代码，未新增生产依赖。

以下仅记录重集成前已知的实现/重集成事实，canonical 结果不在此声明。M12-9 focused（dispatch-contract / canonicalRunner / manifest / docs-consistency 等目标文件）在本次修正中通过；权威 exact-artifact canonical 验证由后续 WAO 验收阶段在打包后执行并给出唯一裁定，本文既不把本地 Lead 接受误写为 canonical 通过，也不预先写下非通过结果，更不声称已发布或已完成 fresh Host 验收。
