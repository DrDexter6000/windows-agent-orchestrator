...
| M10-pre3 | 🔧 准备中 | Disabled deadline + Lead observation wait。**执行截止默认禁用**（M10-pre3 起 worker run 不再用 wall-clock 杀进程，改由 Lead 观察驱动）。新增 MCP `run_wait` tool（只读 long-poll，等待终态或 liveness 摘要，`waitMs` 下限 180s，输入 `runId`/`afterSeq?`/`waitMs?`，输出 `state`/`terminal`/`cursor`/`returnedEarly`/`liveness`/`activityEventCount`/`lastActivityKind`/`ownerHeartbeat`）+ 共享 `runWait.js` application service + liveness 投影 SSOT `ownerLiveness.js`（terminal/progress/process_only/silent）。建立三钟分离：执行截止（默认禁用）/ 后端请求超时（独立）/ Lead 观察等待（`run_wait`），三者解耦互不覆盖。Lead 用 `run_wait` 作主监督原语，取代 busy `run_status` 轮询。M10 整体未完成。 |
