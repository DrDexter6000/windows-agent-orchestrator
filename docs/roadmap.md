...
| M9 | 🔄 in progress | Agent Runtime Control Surface：MCP Server 作为 agent-facing primary 控制面，与 CLI 共享 application services。**M9-0~4 已完成**：registry_list/inventory、run_dispatch/dispatch、run_status/status、run_collect/collect。**M9-5P 已完成**：provider_disconnect 严格签名优先于 no_effect（真实 transcript 回放确认）。**M9-5A/B 已完成**：`runDiagnosis.js` + MCP `run_diagnose`（只读，安全 category + signal event types，无 raw evidence/处方）。inventory + dispatch + status + collect + diagnose 已完成；delivery query/acceptance 仍待做。完成门不变。 |
