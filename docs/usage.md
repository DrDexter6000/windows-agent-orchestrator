...
配套可见性（均非阻塞）：`registry validate` 对已注册但 summary 记录缺 `providerKey` / `lastFullHealthyRunAt` 的 lane 输出 legacy 台账 advisory（半迁移提示，不改 validate 的通过/失败语义）；存量记录**不自动重认证**——补全新维度/新字段的唯一路径是对目标 worker 重跑 `npm run reliability -- --agent <id>`。
