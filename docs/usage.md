...
| **T2** | 控制面交付验证（delivery.verificationCommands） | **全量**（TD-138 Owner 裁定作用域=本层+T3；本层子集路线已被 Owner 拒绝，不因套件变快自动撤销）。全量入命令即声明 `verificationTimeoutMs ≥ 1200000`。**豁免阀门（预登记候选，未生效）**：纯文档/配置类交付改跑 pure 波守卫子集（如 `node --test test/isolation-infra/docs-consistency.test.js`）或声明 `verificationUnavailableReason`——此阀门仅为**待 Owner 批准的候选**（TD-138"子集路线不采用"裁定仍全面有效），批准前 T2 一律全量；豁免只能走 inline 块（executionProfiles 冻结目录无子集 profile）。 |
