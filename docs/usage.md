...
**当前派发策略**（认证结果用于派发选择的边界）正文已完整迁移至 `docs/certification-runbook.md` §认证结果用于派发选择的边界（单向链接；唯一权威）。
...
```jsonc
{
  "agents": {
    ...
    "coder_dsh_acp": {
      ...
      "credentialEnv": "DEEPSEEK_API_KEY",
      "reasoning": { "effort": "low" },      // 可选，仅 low/high/max（wire 实证交集，
                                             // 见生成层 `docs/surface/certification.md` 能力表；其余值被 fail-closed 拒绝）
      "cwd": "D:/projects/my-app"
      // 注意：不配 model/provider 块。provider 组合面固定；model 虽已被 Phase 5
      // 证明同一 ACP 通道可 set，但 WAO 本轮未接线（ACP 的 value 是 provider/model
      // JSON 对，非裸 model.id）——配了仍被 validateAgentPolicy fail-closed 拒绝，
      // 模型取 profile 缺省。
    },
...
**deepseek-harness / deepseek-acp 通道适用限制、`registry validate` 能力交叉 `⚠` 警告与 metrics 判读**正文已完整迁移至 `docs/certification-runbook.md` §DSH 通道适用限制与能力交叉警告（单向链接；唯一权威；含 TD-87 metrics 行为的五态化勘误注记）。

### 接入新模型 / 新运行时（lane 内操作 vs Owner 决策）

本节正文（第一分叉、操作食谱、换模型同步面 checklist、案例一行）已完整迁移至 `docs/certification-runbook.md` §接入新模型 / 新运行时（单向链接；唯一权威）。

### backend 能力对照表（TD-162）

本节指针正文已完整迁移至 `docs/certification-runbook.md` §backend 能力对照表（TD-162）（单向链接）；权威投影仍是生成层 `docs/surface/certification.md`（`npm run gen:certification` 再生成，禁止手改）。

### 上游 harness 原语对照（实测）

本节正文（上游原语表、基线日与刷新期限、读法与勘误注记）已完整迁移至 `docs/certification-runbook.md` §上游 harness 原语对照（实测）（单向链接；唯一权威；刷新期限由 TD-184 守卫钉住）。
### 验证安装
...
smoke / runtime certification（`npm run reliability`）/ drill 转录维护命令正文已完整迁移至 `docs/certification-runbook.md` §认证与转录维护命令（单向链接；唯一权威）。

### 认证正文总落点（certification runbook）

认证操作正文已于 2026-09-26 **文件级全文迁移**至 `docs/certification-runbook.md`（Owner 授权；原文完整移动，非索引/节选路线）：认证结果用于派发选择的边界、DSH 通道适用限制与能力交叉警告、接入新模型 / 新运行时、backend 能力对照表（TD-162 指针）、上游 harness 原语对照（实测）、认证与转录维护命令、delta 认证规程、认证检查结果五态与能力轴分层、单次派发覆盖（场景 1b / 1c，含 MCP 覆盖特有规则）、retry 的 per-dispatch 覆盖继承、认证与当前就绪（历史认证 ≠ 当前凭据/授权/容量）。它是 `seat-certify` 行动面的必读成员（`docs/ssot.md` §0.1）；本页各原位置只保留具名标题与单向链接，不保留第二份当前正文。既有「认证节」类指向（`npm run reliability` 参数说明、backend 能力轴分层骨架等）经本节与下列标题到达新正文。

### delta 认证规程（lane 架构，ADR-0025 批次 3）

本节正文已完整迁移至 `docs/certification-runbook.md` §delta 认证规程（单向链接；唯一权威）。

### 认证检查结果五态与能力轴分层（ADR-0032 §8，2026-09-21）

本节正文（五态判定纪律、`reportsCommandExitCode` 诚实声明、运行时身份入账、能力轴分层骨架、认证证据绑定执行画像与只读适用性三态、更新触发器与执行人）已完整迁移至 `docs/certification-runbook.md` §认证检查结果五态与能力轴分层（单向链接；唯一权威；`harness-certify` 行动面读本节时经此链接跟进新正文）。
...
本节正文（含 MCP `run_dispatch` 的 `model` 覆盖特有规则）已完整迁移至 `docs/certification-runbook.md` §场景 1b（单向链接；唯一权威）。
### 场景 1c：单次派发换推理力度（--reasoning，R11-1）

本节正文（含 MCP `run_dispatch` 的 `reasoning` 覆盖特有规则）已完整迁移至 `docs/certification-runbook.md` §场景 1c（单向链接；唯一权威）。
...
| 层 | 场景 | 要求 |
|---|---|---|
| **T0** | 开发回路（改一处验一处） | `node scripts/wao-node.cjs --test --test-timeout=600000 test/<file>`——**必须显式带 `--test-timeout`**：裸 focused 命令不经过 canonical runner 的看门狗（TD-165），漏带等于把无界挂死引回来。注意这不等于完整的进程树兜底（canonical 的波级 /T 树杀只在 T2/T3 存在）。直跑若因子进程经 PATH 的 v24 node 被版本守卫拒（复跑即现），设 `WAO_SKIP_VERSION_GUARD=1`——canonical runner 构造测试环境时注入的同款豁免（scripts/canonical-test.mjs childEnv）。大批量 focused 不与 T2/T3 同机并行（资源争用即 TD-130 形状）。 |
| **T1** | worker 交付前自检 | **focused，不跑全量**（"全量自检"是历史惯例，从未是成文规则）。范围按改动类型：局部 `src/**` = 新改测试 + 直接消费者的既有回归（任务书列具体文件与理由；注意资源类目 ≠ 影响范围——`src/transcript.js` 同时被 pure 与 filesystem 波测试消费）；`test/**` = 被改文件自身（共享夹具改动覆盖其消费者）；新增/移动/删除测试 = manifest 同步 + 不能只跑新文件；runner/reporter/shim/公共夹具 = 对应元测试 + 跨执行边界回归；文档/工具面 = 对应文档守卫 + `gen:surface` 一致性。**任务书把 worker 自检命令与 `delivery.verificationCommands` 分开写**（同一条 `npm test` 写两处 = 同批两次全量）。T1 汇报如实写"这些文件通过，其余未测"——focused 命令同 T0 必须显式带 `--test-timeout=600000`；漏检会在同批 T2 全量上变红（在既有测试覆盖范围内）。 |
| **T2** | 控制面交付验证（delivery.verificationCommands） | **全量**（TD-138 Owner 裁定作用域=本层+T3；本层子集路线已被 Owner 拒绝，不因套件变快自动撤销）。全量入命令即声明 `verificationTimeoutMs ≥ 1200000`。**纯文档交付的逐案豁免（Owner 2026-09-19 批准；无常设规则）**：**config 一律不豁免**（角色合同/配置被运行时真实消费——roleContract.js/registry.js 解析注入）。遇到真正的纯文档交付时 Lead **逐案**裁定 inline 守卫子集，四要件：①判定依据=控制面打包时的**实际改动路径**全属文档白名单（`*.md`、`docs/**`——排除 `config/**`、`test/manifest.json`、一切代码文件），不是 worker 自称；②守卫子集**指名具体文件+选择理由**（文档守卫散布多个波——docs-consistency 实为 worktree 波非 pure 波；派生法：凡源码读 SKILL.md/AGENT_ONBOARDING.md/docs/** 的测试文件即文档守卫，勿手写常备清单）；③豁免命令随 delivery 持久化可审计；④T3 集成终验照跑全量兜底。`verificationUnavailableReason` 是"零验证"声明（结果记 unavailable 非 passed），不得作为缩减验证的替身。TD-138 裁定不变——本节是逐案判断通道，不是常设豁免规则（executionProfiles 冻结目录无子集 profile，豁免只走 inline 块）。 |
...
**retry 的 per-dispatch 覆盖继承**（R12/R13：任务文本 runId 绑定取法、继承范围、坏持久化值 fail-closed、`--model`/`--reasoning` 替换 flag、reuse 形状与全新 provider session 边界）正文已完整迁移至 `docs/certification-runbook.md` §retry 的 per-dispatch 覆盖继承（单向链接；唯一权威）。
...
**TD-111 certification advisory context**（`certificationReasonCode` / `certificationLastHealthyAt`）正文已完整迁移至 `docs/certification-runbook.md` §认证与当前就绪（单向链接；唯一权威）。
...
**M11-7 凭据可用性**（`credentialAvailability`）与 **M12-6 FR-02 provider readiness 真相**（`providerReadiness` 字段含义与语义铁律：历史认证与当前凭据、授权、容量的区别）正文已完整迁移至 `docs/certification-runbook.md` §认证与当前就绪（单向链接；唯一权威）。
...
`delivery` 可选。`verificationCommands` 与 `verificationUnavailableReason` 二选一（互斥）。WAO 强制 persistent worktree isolation——模型不能传 `isolate`（隔离不是可选输入）。 registry certification 的定位（advisory 证据、非 permission gate、显式 CLI `--require-certified` opt-in 门与 MCP 差异）正文见 `docs/certification-runbook.md` §认证与当前就绪（单向链接；唯一权威）。

**行为变更（TD-131/TD-132 认证门收口 + R23-C 认证身份维度补全）**正文（`--require-certified` 完整门合同：身份四元组比对、`lastFullHealthyRunAt` 新鲜度与 legacy 回落、`manualOverride:"cleared"` Owner 旁路、MCP `requireCertified:false` 差异、配套可见性 advisory）已完整迁移至 `docs/certification-runbook.md` §认证与当前就绪（单向链接；唯一权威）。
...
**R10-A 单次模型覆盖（可选，`model`）**的 MCP 特有规则已并入 `docs/certification-runbook.md` §场景 1b（与 CLI 覆盖正文合并维护；单向链接；唯一权威）。

**R11-1 单次推理力度覆盖（可选，`reasoning`）**的 MCP 特有规则已并入 `docs/certification-runbook.md` §场景 1c（与 CLI 覆盖正文合并维护；单向链接；唯一权威）。
...
`lead_preflight` 让 Lead 一次调用完成 workspace 选择/确认 + worker credential 可用性 + active-run 查询，替代机械地依次调用 `workspace_select`/`workspace_status` + `registry_list` + `runs_list`。**ADVISORY ONLY，不是 gate**：每项检查独立结算（一项失败不吞其他），输出是事实供 Lead 判断，绝不自动中止——不产生 permit/token/approval 状态，`run_dispatch`/`workspace_select`/`registry_list`/`runs_list` 不依赖它曾成功。`complete` 仅表示机械事实（registry 可读、必需凭据 env 名存在/不存在、active run 可数）是否可读取，**不是** authenticated/entitled/live-checked 的证明，也不是"是否应派发"的裁定 认证/授权证明边界（M12-6 FR-02：preflight 完成永不意味着任何 worker 已被认证/授权/做过 live check）见 `docs/certification-runbook.md` §认证与当前就绪（单向链接；唯一权威）。
...
不返回 `PASS`/`FAIL`；check-level 状态为 `observed`/`warning`/`unknown`。`manualChecks` 指向原始 MCP 工具，允许 Lead 独立复核（与聚合结论不同时，Lead 可依据直接证据继续并记录 friction）。Active run、conditional worker、dirty workspace 的派发边界见 `docs/certification-runbook.md` §认证与当前就绪（单向链接）。
...
**R21 内容绑定（TD-128 W2 升格修复，2026-08-19）**：process 路径的**重建条目**自 R21 起只重建本 run 信封绑定的 `run.event`——`run_collect`（full/compact）与 `run_await_result` 的 compact 结果共用同一重建 SSOT（`reconstructItemsFromEvents`，调用侧收窄、函数本体不动），尾部追加的外 run/伪造行不再被重建进输出（messages/evidenceCounts/itemCount 等机器消费字段不含不可归属内容）。合法全绑定 transcript 上过滤器恒等（输出零变化）。collect 侧无 legacy 腿（上游会话门已拒全无信封形状）；`run_await_result` compact 侧全无信封 → 绑定后重建为空（该形状经终态门后不可达，纵深防御）。上文"外 run 伪造行对 collect 不可见"自此覆盖**会话事实与重建内容**两面。
...
**R20 读绑定（TD-128，2026-08-19）——伪终态压制解除（候选门控 + 孤儿恢复）**：`run_delivery`（point-in-time 与 `--wait-ms` readiness 两条路径）投影的 `terminalState` 自 R20 起**只由本 run 自身信封事件计算**（尾部追加的外 run/伪造终态行不可见；不可归属时按 `pending` 非终态路径，绝不凭不可归属事件投影终态）。两个此前被伪终态尾条**压制**的面随之解除：① 候选投影（`backend_failed` / `process_missing` / isolation 结算门）自 R20 起只由本 run 自身事件门控——绑定前外 run `completed` 尾条曾把真实 failed run 读成 completed、压制恢复候选；外 run `failed` 尾条曾为非 failed run 凭空武装候选门，两个方向都已关闭；② daemon 的孤儿恢复（`daemon start --resume-on-start` / `daemon list` 统一视图）：真实在飞 + 外 run 伪终态尾条 + owner 已死的 run，绑定前被终态门永久压制（永不 resume、从统一视图消失），绑定后按本 run 自身非终态诚实放行给 owner 心跳关（关卡序不变：仍先终态跳过、再 owner 心跳判活）。合法全绑定 transcript 上过滤器恒等——合法路径与既有输出零变化；诚实边界同 R13-C 统一口径：绑定只杀跨 run 注入与错读，同 runId 伪造追加 = `runs/` 写权限攻击面，读侧无解。

**R20 交付丢失向量关闭（TD-128，2026-08-19）**：runManager 采纳"外部终态"（stop/abort 等并发写入的终态，防止被覆盖）前的读取自 R20 起绑定到本 run runId——append-only 尾部的外 run 伪终态行不再被采纳。此前该无绑定采纳是**交付丢失向量**：交付模式 run 完成时若尾部存在外 run 伪终态行，打包被跳过、run 以伪终态收场而**交付物从未打包**；绑定后交付照常（合法 stop/abort 写入车道全带 runId 信封，绑定对合法写入透明）。daemon `status` 兜底、后台 runner 启动失败/解析失败的 `run.error`+`failed` 落盘同款绑定（伪终态尾条不再压制失败事实落盘）。
...
**R21 读绑定收官（TD-128，2026-08-19）——行 state/terminal/updatedAt 全路径绑定**：`runs_list` 每行的 `state`/`terminal`/`updatedAt` 只由本 run 自身信封事件计算（文件名 stem 即权威 runId）——MCP 查询缓存路径（`lead_preflight` 同一缓存）与 CLI 直读路径同语义（R20 只绑定了直读分支的 state/terminal，缓存路径仍历史读法）。尾部追加的外 run/伪造行不再翻转行状态/terminal 分类，也不再拉动 `updatedAt` 及由其派生的排序与 historyRange 窗。全无信封的 legacy transcript 保持历史读法；任一事件带信封即严格绑定，零绑定事件（整份只有外 run 信封行）降级 pending / `updatedAt:null`（宁可可见地缺事实）。`agentId` 仍为 events[0] 全量派生（append-only 首写不变量——按 TD-128 events[0] 类级登记维持）。
