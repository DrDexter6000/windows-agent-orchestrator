...
- **M12-10 静态工具面（protocol-neutral tool profile）**：一个 tool profile 是 WAO 全部 MCP 工具的**封闭、Host 中立子集**，且**仅在 server 构造时选定**（startup-fixed，连接建立后冻结、运行期不可变）。它是**呈现层（presentation surface）**——只决定一个 Host 看到并能调用哪些工具；它**不是**权限层、**不是**路由层、**不是** Host/runtime-name 适配器：代码里**没有任何**按 Claude/Codex/Kimi/OpenCode 的分支。每个工具的 `name`/`description`/`inputSchema`/`outputSchema`/`annotations` 在所有 profile 间**逐字节相同**；profile 只选择集合。封闭集只有两个值 `full`（23 个工具，默认——对任何不传 flag 的既有 Host 配置**零行为变化**）与 `lead`（18 个工具的显式 opt-in 子集，隐藏 `workspace_select`/`run_dispatch_contract_check`/`run_wait`/`playbook_list`/`playbook_get` 五个已被 lead 集合内其他工具覆盖的工具；所有 `DRILLDOWN_TOOLS` 命名的工具（6 个）均在 lead 集合内，故 `availableDrilldowns` 永不广告一个被隐藏的工具）。`--tool-profile full|lead` 是**加性** stdio 参数，不改 `--registry`/`--run-dir`/`--workspace-root` 语义；缺失/重复/空值/未知值一律 fail closed（固定安全致命文本，不回显值，不静默回退 full）。`createWaoMcpServer({toolProfile})` 缺省解析为 full；单一来源在 `src/mcp/toolProfiles.js`（闭集定义 + 一致性不变量），server.js 用统一注册 helper 按 profile 决定是否挂载每个工具的 handle（不使用 SDK 私有字段），stdio.js 的 arg parser 用同一闭集做合法性校验——parser、server、test 三者不会漂移。切换 profile 需重启 Host；profile 不改变任何工具的输出校验、read/write 注解、Lead 决策权或全部能力——这些在两个 profile 下完全一致。工具描述在 M12-10 做了手工精简（去掉版本日志、重复教程、schema 已有的枚举列表；保留用途、关键副作用/只读、Lead 决策边界、核心 fail-closed/安全事实；详情见 `SKILL.md` 权威）。
...
│   ├── toolProfiles.js       #   M12-10 静态工具面 SSOT：full(23,默认)/lead(18) 封闭集 + resolveToolProfile/exposedToolSet + 一致性不变量；server.js/stdio.js/test 共用，无 Host/runtime 分支
│   └── stdio.js              #   stdio production entrypoint（StdioServerTransport，npm run mcp，--workspace-root，--tool-profile full|lead）
