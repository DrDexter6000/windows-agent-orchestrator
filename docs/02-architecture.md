...
│   ├── runContinue.js        #   Lead 授权修正续跑 service（M12-7，CLI+MCP 共用）：对终态 continuable delivery 续 ONE 修正回合，复用父 run 的 retained worktree + 续 provider 会话（turn:resume，同一 opaque uuid）；9 道只读资格检查 + closed-set rejectionReason 先于任何 mutation；从不推断 correction/scope/retry/accept
