...
> 状态：✅ 契约层稳定。本文档定义的接口契约 / 数据模型 / 状态机 / 模块边界已由 M0–M10 实现落地并经测试固化。
...
```
MCP Adapter ─┐
             ├→ Application Services (M9 minimum Lead use-case layer — complete)
CLI Adapter ─┘                 ↓
...
- Application Services 层是 M9 最小 Lead 闭环的 use-case 层（已完成）。**已提取的共享 services**：`registryInventory.js`（M9-0）、`runDispatch.js`（M9-2A）、`runStatus.js`（M9-3A，只读）、`runCollect.js`（M9-4A，非只读）、`runDiagnosis.js`（M9-5A，只读）、`runDelivery.js`（M9-6A，只读查询 + 持久决策，委托 `tryAppendDecision` 原子 first-decision-wins；M11-10 增 `getRunDeliveryReadiness` 可选 bounded 只读 readiness wait + `projectDeliveryReadiness` 闭集投影，复用 `validateDeliveryFacts` SSOT，CLI/MCP 共用同一 service；M12-13 增 `isolation_failed` 闭集状态 + `isolationFailure:{code}` 安全投影 + `projectIsolationViolationCode`——终端 delivery-requested run 且**唯一**较高优先级 delivery 事实为恰好一条 run-bound `run.isolation_violation(code=workdir_escape)` 时 settle 为 `isolation_failed`，与 packaging failure（`deliveryFailure`）严格分离，绝不带 candidateInventory/repackage/salvage/retry/stop/decision 面；facts 优先于隔离事实，缺失/多条/跨 run/malformed code 一律 fail-closed `ambiguous`）、`workspaceBinding.js`（M10-pre2，host-authorized workspace proof SSOT）、`runStop.js`（M10 P0-2，workspace-bound 失控 worker 停止，CLI `stop` 与 MCP 共用）、`runList.js`（M10 P0-3，workspace-bound run 列表，CLI `runs list` 与 MCP 共用）、`runWorkspaceOwnership.js`（M10 P0-3，run 是否属于当前 workspace 的判定 SSOT）、`runWait.js`（M10-pre3，long-poll 终态/活性等待，只读，MCP 共用）、`runAwaitResult.js`（M12-3，只读 advisory composite：bounded wait + truthful observation + terminal compact，snapshot-only 无 serve fetch，复用 `runWait` 的 `summarizeLiveness` + `runCollect` 的 `reconstructItemsFromEvents` + `runCollectProjection` 的 compact SSOT，零 audit append；M12-6 FR-08 增必填可空闭集 `readFailureReason`——`READ_FAILURE_REASONS` 单一 SSOT，MCP schema enum 由它构建，read_failure 时给出 transcript_parse_failed/legacy_event_shape/snapshot_unavailable 机器码，observed 为 null，绝不泄漏错误原文）、`ownerLiveness.js`（M10-pre3，run liveness 投影 SSOT：terminal/progress/process_only/silent，`runWait`/`runAwaitResult` 共用）、`runDeliveryReview.js`（M11-3，exact delivery commit proof + bounded/redacted diff projection，CLI/MCP 共用）、`runCollectProjection.js`（M11-4，run_collect 的共享安全投影 + cursor codec + 分页，CLI 续读与 MCP 委托同一投影，无第二算法）、`sessionWorkspace.js`（M11-6，Lead 会话级工作区选择纯验证内核，委托 `workspaceBinding.js` 的 `proveWorkspace` SSOT；无状态——per-MCP-server 会话状态由 server 闭包持有，application 层不存全局/持久状态）、`runDrilldowns.js`（M12-8B，有界 Lead 渐进式披露元数据 SSOT：静态闭集 catalog（tool/view/detail/purpose/reveals/cost/readOnly 七键）+ 纯事实选择规则 + 硬上限 `DRILLDOWN_MAX_ENTRIES`/`DRILLDOWN_SERIALIZED_MAX_BYTES`/`DRILLDOWN_FIELD_MAX_LEN`；`run_await_result`/`run_status`/`run_diagnose`/`run_collect`/`run_delivery`/`run_activity` 六个观察工具的输出携带（schema REQUIRED）`availableDrilldowns`，MCP schema 枚举与上限由同一模块导出派生，无第二份列表；只披露不调用、不决策、不广告 mutation/control 工具，`readOnly` 按工具如实（run_collect 三条为 false，余为 true），字段永不含 transcript/provider/仓库文本）。CLI `runs delivery` / `runs diagnose` / `collect` / `status` 均委托共享 service。非 M9 use-case orchestration 仍在 `src/commands/*.js` 中。
...
- M9 已暴露 MCP `registry_list`（只读）、`run_dispatch`（destructive，delivery-capable）、`run_status`（只读）、`run_collect`（非只读非幂等）、`run_diagnose`（只读安全诊断）、`run_delivery`（只读 delivery 查询）、`run_delivery_decide`（destructive，持久 Lead 决策，first-decision-wins）。Codex + Claude Code/Fable 两个不同 Lead Runtime 真实 MCP dogfood 已通过（runId: `run_20260715122607417p5fbue` / `run_20260715124226755a97el2`）。M9 最小 Lead 闭环正式完成。M10-pre2 新增 `workspace_status`（只读，host-authorized workspace binding 状态查询）并使 `run_dispatch` 在调用 shared service 前重新解析并证明 workspace——证明后的 canonical Git root 作为 server-owned `cwd` 传入，模型不能通过 tool argument 提供任意路径。M11-6 新增 `workspace_select`（会话级，Lead 选择工作 Git 项目，`lead_session` 来源，最高优先级）——workspace 解析优先级变为 `lead_session` > `mcp_root` > `server_config` > fail-closed；Lead 无需 Human Owner bind、项目配置或重启即可在当前会话选择项目并派工。M10 P0-2 新增 `run_stop`（destructive，workspace-bound，停止失控 worker），委托共享 application service `runStop.js`（CLI `stop` 与 MCP 共用同一 service）。M10 P0-3 新增 `runs_list`（只读，project-bound run 列表，用于 recovery），委托共享 application service `runList.js`（workspace ownership 由 `runWorkspaceOwnership.js` 判定）。M10-pre3 新增 `run_wait`（只读 long-poll，终态/活性等待），委托共享 application service `runWait.js`（liveness 投影 SSOT 在 `ownerLiveness.js`）。M11-10 给 `run_delivery` 增加可选 `waitMs`（共享常量锁定 `[1000,300000]` ms），触发 bounded 只读 readiness wait 并返回严格闭集 `readiness`（`waiting_for_packaging|waiting_for_verification|reviewable|packaging_failed|isolation_failed|not_requested|ambiguous`，`isolation_failed` 为 M12-13 新增，见 `runDelivery.js` 条目）；CLI `runs delivery --wait-ms N` 与 MCP 委托同一份 `getRunDeliveryReadiness` service——MCP 不解析 transcript、不 shell-out CLI，`run_delivery_review` 的 exact-proof/安全投影/错误边界未被放松，省略 `waitMs` 时 point-in-time 输出向后兼容。M12-3B 新增 `run_delivery_review_bundle`：MCP adapter 机械组合同一 readiness service、同一 `run_delivery` 安全投影和同一单文件 review service/投影；默认等待 270 秒、settled early return，只在 `reviewable` 时读取 Lead 指定的一页，并绑定 delivery/review commit 与 changed-file count。它不选择或遍历文件/cursor、不做语义判断、不作 accept/reject；原子工具保持可用。M12-6 Package 3B 新增 `run_delivery_reverify`（第 20 个 tool）：委托共享 application service `runDeliveryReverify.js`（CLI `runs delivery reverify` 与 MCP 共用同一 service），对同一未变 delivery commit 做审计式重验证——原始 assertion 命令逐字节重跑不可修改，Lead 只能追加 setup 命令，可重入/崩溃安全收敛为至多一条 outcome，永不自动 accept/reject；原始 verification 与 `run_delivery` 投影的 `originalVerificationStatus`/`effectiveVerificationStatus`/`reverify` 链事实一并保留。M12-7 新增 `run_continue`（第 21 个 tool）与 `run_dispatch` 的 delivery-only 顶层 `continuable` 选项：Lead 审阅一个终态 continuable delivery、发现窄缺陷后，显式授权对其续 ONE 修正回合——WAO 创建新 run/transcript、复用父 run 的 retained worktree、以 `turn:resume` 续同一 provider-native 会话（opaque uuid 由 Lead session + canonical workspace + canonical agentId + rootRunId 派生，跨一条续谱复用，非 project-wide coder 复用），并打包新 child delivery。`continuable:true` 在 dispatch 时以 `run_lineage`/`turn:first` 建立续谱根。closed-set 只读资格检查覆盖输入与 delivery、父 run 身份/终态/acceptance、lineage/provider session、workspace/delivery、worker backend+model 配置、backend 能力、retained worktree 与并发占用，均先于 mutation；静态 argv/credential 亦先检查。retained-worktree 转换幂等且事务化，spawn 前失败恢复父 worktree、清理 child transcript 并释放 claim；第二次证明发现外部漂移时不覆盖外部状态（父 commit 对象永不删除，仍可按 SHA 审阅）。WAO 从不推断 correction/scope/verification/retry/acceptance——child delivery 的 review/accept/reject 仍归 Lead。opaque uuid、Lead id、workspace 路径、active lineage runId 永不出现在 MCP 输出。
- M12-8 将可观测性分成两个消费者面，但共用同一事实投影：`runActivity.js` 单快照读取并做 workspace/runId 绑定，`runActivityProjection.js` 负责七类活动事件、secret redaction、控制字符清洗、有界 excerpt 与冻结 cursor；MCP `run_activity` 使用 `audience:"lead"` 的紧凑页面，Owner 组合 service `ownerDashboard.js` 使用更大的 `audience:"owner"` 页面。`runDrilldowns.js` 只向 Lead 返回静态 `availableDrilldowns` 提示，不自动下钻。`ownerDashboardServer.js` 是独立的人类观察边界：只绑定 `127.0.0.1`、每进程随机 bearer、严格 GET/query/CSP/no-store，且只复用上述 application services；`runs dashboard --web` 启动固定静态 UI。Owner 页面不会 stop/retry/continue/repackage/decide，也不改变 Lead 的语义裁决权。两种消费者都不直接解析 JSONL，详细合同与命令见 `docs/usage.md`。M12-14：`runScopeObservation.js` 在同一快照上投影 advisory `scopeObservation`（闭集 `within_declared_paths | outside_declared_paths | unknown`，`source:"transcript_file_events"`，`observedFileCount` + 上限 25 条的脱敏 `outsidePaths`）——`complete:true` 的准确语义：快照已**终态**，且其中每条确认的 `file_written` 都能在**恰好一个有效合同权威**（绑定 runId 的 `run.started` 绝对 worktreePath + 非空合法 `delivery.allowedPaths`）下求值；它**不**证明文件系统完整性、语义正确性、交付验证或 Lead 验收，也**不**表示 worker 仍在运行（`complete` 的前提是快照终态）。未终态或任一确认路径不可求值 → `unknown`/`complete:false`。Windows 上绝对路径 containment 证明按平台大小写折叠；输出路径先 exact-secret 脱敏再 bound。
- **M12-16 运行中显式纠正**：`runCorrection.js` 是 workspace-bound application service；`run_dispatch(correctable:true)` 只在 backend 明确声明 in-flight correction 能力时建立 durable correction queue，`run_correct` 以 bounded `correctionId` + prompt 把 Lead 明确指令追加到原 run，再由 runner 串行 claim 并送入同一 provider 进程。requested/claimed/delivered/delivery_failed/rejected 是 transcript 事实；`queued` 不等于 delivered，`delivered` 也不等于模型已理解或执行。该能力不创建 child run、不换 worktree、不扩大 delivery contract、不自动 stop/retry/accept；语义判断仍归 Lead。普通未 opt-in dispatch 字节兼容。
- **M12-17 submitted-stage 与 Owner 详情/通知**：`runStageProjection.js` 从单 run transcript 快照投影 `accepted | spawned | active | terminal | unknown` 的事实阶段，只由 durable submitted/started/activity/terminal facts建立，不预测进度、不判断产品语义。Owner Dashboard 复用同一 application projection 展示选中 run 的 backend/stage/terminal/event count/scope/liveness/timeline；浏览器 Notification 仅在人类点击启用、权限允许、且观察到非终态→终态时发送一次固定安全通知。所有异步详情响应用 `runId + selection epoch` 绑定，旧 selection 不得污染当前视图；页面仍为 loopback、bearer、GET-only 的纯观察面。
- **M12-20 Owner Dashboard active-first / history-on-demand**：Owner 观察面默认 Active——只列出当前有新鲜 owner 心跳、经证明 active 的 run。Active 的候选集是当前 `.owner-*` 租约文件——候选枚举只 `readdirSync` 一次 runDir 目录（成本随目录条目总数增长、包括历史 transcript，但仅一次目录读），随后只有当前租约候选者的 transcript 被打开/解析、做 per-run workspace 验证与 `ownerLiveness` 心跳检查（昂贵工作是 O(当前租约数)，无任何历史 transcript 被打开/解析/验证）；`listRuns` 仍是单一应用 SSOT，以 `scanScope`（`active` 枚举租约候选并只保留 proven-active、`history` 按 transcript 派生 `updatedAt` 做有界闭集范围过滤、缺省行为字节不变）+ `historyRange` 扩展，**不是**第二个分类器，并复用既有 `ownerLiveness` 心跳真相与 closed-set activity 分类。Active 响应固定标注 `scanScope=active` 且不携带 `unresolvedCount`（active/history scope 不做全库存 unresolved 分类，故恒缺失而非 0）。历史是显式有界按需：`1h/24h/7d` 预设或有界自定义 from/to（上限 7d），由 `ownerDashboardServer.js` HTTP 闭集严格校验；长驻看板服务进程复用 M12-18 的元数据校验 summary 缓存做热读，**无第二个持久索引**。前端 runs 列表绑定 runs-mode epoch（类比 selection epoch），迟到的 Active/History 响应不得覆盖当前模式或更新的查询；run 从 active 集或有界历史窗口消失绝不构成终态迁移、不触发通知。MCP `lead_preflight`/`runs_list` 热路径、CLI 文本看板、transcript writer、delivery 语义与验收权威均未改动。详细命令与合同见 `docs/usage.md`。
- **M12-10 冻结工具面（progressive-disclosure correction）**：WAO 暴露**恰好 22 个 always-registered MCP 工具**——无 profile、无 startup flag、无 restart-to-recover：每个操作工具对连接整个生命周期都可独立调用。这是**呈现层**——它**不是**权限层、**不是**路由层、**不是** Host/runtime-name 适配器：代码里**没有任何**按 Claude/Codex/Kimi/OpenCode 的分支，也不依赖 `tools/list_changed` 或运行期动态注册。每个工具的 `name`/`description`/`inputSchema`/`outputSchema`/`annotations` 固定且逐字节稳定。单一冻结来源在 `src/mcp/toolSurface.js`（22 个名字的 frozen 数组 + 唯一性/计数/无 playbook 工具的模块加载不变量；server.js 在构造期对实际注册序列做 deepEqual 自检，绑定 production 到该 SSOT，无 Host/runtime 分支）。22 = 原 23 减去两个原 playbook 工具、M12-16 增 `run_correct`——built-in playbook catalog **整体移出工具面**，改为 MCP resources（`wao://playbooks` summary + `wao://playbooks/{id}` 详情，委托同一 `application/playbookCatalog.js` SSOT）。stdio argv parser 将残留的旧 profile 参数视为普通未知 flag 忽略——不解析值、不出现在输出、不改变 server 面、不失败启动；`--registry`/`--run-dir`/`--workspace-root` 解析逐字节不变。所有 `DRILLDOWN_TOOLS` 闭集成员（`run_status`/`run_activity`/`run_collect`/`run_delivery`/`run_delivery_review`/`run_diagnose`）均在 22 集合内，故 `availableDrilldowns` 渐进式披露提示永不广告一个不可调用的工具；它只披露、不自动调用、不决策、不广告 mutation/control 工具。没有任何能力被永久移除或弱化——一个永不重启的 Host 保留全部操作能力。
- **M12-12 Self-Describing Results**：恰好四个 standalone 成功结果——`run_wait`/`run_await_result`/`run_delivery`/`run_diagnose`——携带 REQUIRED `semanticNotes`（1..4 条），每条恰好三键 `{id, meaning, doesNotMean}`：`id` 是冻结的 namespaced 闭集值，`meaning` 是一句确定性事实，`doesNotMean` 是 0..2 条确定性非含义。无 `scope` 字段、无 per-entry `semanticsRef`，详情 URI 机械派生 `wao://semantics/{id}`。单一纯、provider-neutral 的 SSOT 在 `src/application/runSemanticsNotes.js`（冻结静态目录 + 纯 selector，导入任何东西、无 runtime/backend-name 分支、无动态模型文本、永不回显 transcript/provider/path/prompt/command/session）。目录与 selector 的事实输入来自既有闭集（`OBSERVATION_OUTCOMES`/`TERMINATION_SOURCES`/`DIAGNOSIS_CATEGORIES`/delivery readiness·verification）；未知/缺失值退化到显式安全 fallback note（见下），不新增事实闭集。强制点在 application 层：1..4 条、每串有界、id 唯一、精确三键、目录成员、≤2048 字节 UTF-8 序列化上限（空数组 fail closed）；未知 source tool fail closed，未知/future/缺失事实值退化到显式安全闭集 fallback note（`observation.unknown`/`termination.unknown`/`diagnosis.unknown`），永不动态回显（未知/缺失 observation outcome 不再误归 `observation.point_in_time`）。selector 优先级确定：`observation.read_failure` 排除任何终止 note；终止 note 区分 completion/execution_deadline/manual/provider/backend/control_plane/unknown；delivery note 明确 verification passed **不是** Lead acceptance 且**永不建议** accept/reject/repackage，已请求但未 settle（`deliveryRequested:true`/`deliveryAvailable:false`/无 verification·failure）投影为 `delivery.waiting` 而非 `not_requested`；diagnosis note 只陈述事实、不给处方。新增只读 MCP resources：`wao://semantics`（summary：每个 id + meaning）与 `wao://playbooks` 同等；`wao://semantics/{id}` 为 ResourceTemplate 详情（**不**把每个 detail id 注册为独立静态 resource）。未知/畸形 id 返回固定安全文本且**不回显请求 id**。MCP adapter 在严格 output parse 前立即 attach notes；output schema 的 `id` 用**有界 shape**（冻结 namespace pattern + 长度，派生自同一 SSOT）而非完整 catalog enum——避免每个 outputSchema 重复序列化 33+ id 的 enum 撑大 tools/list wire；application validator（`ID_SET`）仍是 catalog 成员的唯一权威（每个 selector 只发 catalog id）。错误响应保持固定且无 structuredContent；`run_delivery_review_bundle` 嵌入的 nested delivery BASE **不含** semanticNotes。工具面恰好 22（M12-16 增 `run_correct`），既有 playbook resources 与工具描述不受影响（描述只说 semanticNotes 自解释当前事实 + 详情在 `wao://semantics/{id}`，不复制目录措辞）。
...
- Worker 可以接收稳定 role contract 和必要安全/运行时约束。M11-5（TD-89 修复 + Package A2/C 收口）后：角色合同注入由 **backend 能力声明** 驱动——RunManager 仅当 `backend.supportsRoleContract === true` 才允许带角色（严格相等，字符串/数字/对象/null/undefined 一律 fail closed），**不按 runtime 名称分支**。`systemPrompt` 路径由共享 `roleContract.js` 的 `resolveRoleContractPath` **相对 WAO 安装根**（从模块 URL 推导，非 `process.cwd()`）解析，所以同一份全局 registry + 角色文件可从任意目标项目目录使用。**加载时序**：start 在创建 transcript 和 spawn 前加载验证角色；resume 在读取既有 transcript 后、任何 append/spawn 前加载（失败时 transcript 字节不变）。不支持注入的 backend 配了 `systemPrompt` 在这两个时点 fail-closed（start 抛错、resume 显式抛错而非静默 return null）。四个当前 backend 各自注入：claude-code 用 `--append-system-prompt <内容>`（恰好一次，用内容不用路径以消除 TOCTOU）；codex 用 `-c developer_instructions=<安全 TOML 字符串>`（append 到 developer message，不替换 base instructions）；kimi-code 用固定分隔组合 role+task（role 在前 task 在后各恰好一次——**不是系统级权限隔离**，是 prompt 级引导）；OpenCode 1.18+ 用 message API 原生 `system` 字段，task 仍只在 user text part 出现一次。runtime 差异只存在于 backend 内，共享 orchestration 不按 runtime 分支。Lead/model 不能通过 MCP/CLI 覆盖角色（run_dispatch strict schema 不接受 `systemPrompt`/`roleContract`/`rolePath`）。**WAO 不把角色合同保存为 `prompt.sent`/控制面输入**——transcript 只持久化原始 task prompt。（注意：worker 输出可能在回答中引用或复述角色内容，这由模型决定，因此不能宣称 transcript 永远不含角色正文。）**M11-8B canonical worker identity**：身份闭集 SSOT 是 `src/canonicalAgentId.js`（`isValidCanonicalAgentId` + `safeProjectAgentId`：合法字符仅 `[A-Za-z0-9._-]`、长度 1..128；dependency-free、root-level）。`roleContract.js` 的 `composeRoleContractWithIdentity` 是唯一组合函数，**只接受已验证的 canonical id**——非法 id（含 newline/whitespace/injection）不得以任何形式进入模型提示词（不"压成同一行"，而是直接拒绝加身份头，只返回角色正文）。start 和 resume 都走它，结果一致、角色合同恰好注入一次。只有本就配了角色合同的 agent 加身份头；未配 systemPrompt 的行为不变。身份头不伪装成 Lead task prompt（`prompt.sent` 仍只存原始 task prompt）。registry normalization（`registry.js`）用同一 SSOT：非法 registry id 在 transcript/spawn 前以**固定安全错误**拒绝（不回显恶意 id；是配置合法性校验，不是 Lead workflow gate）。结构化身份提取是 `transcript.js` 的 `extractCanonicalAgentId(events, expectedRunId)`：**只有每个事件都具备与 `expectedRunId` 一致的 `runId` 且同一个合法 canonical agentId** 才返回该 id；缺失、冲突、非法或跨 run 一律返回 `"unknown"`（不抛错、不伪造身份、不是自动停止门）。status/wait/collect 传入其请求 runId，并复用各自已读的同一份 transcript 快照（身份不额外读 transcript/registry/文件系统）。`run_dispatch`/`run_status`/`run_wait`/`run_collect` 的 MCP output schema 都用共享长度/字符合同声明 `agentId`、根对象 strict，并返回 `OUTPUT.parse(payload)` 的结果；malformed/extra/超长/injection service output 折叠为既有固定安全错误。**M12-6 WQ worker evidence discipline**：同一 SSOT 组合函数 `composeRoleContractWithIdentity` 在身份头与角色正文之间恰好一次地注入一个固定的控制面所有证据纪律块（导出常量 `WORKER_EVIDENCE_DISCIPLINE`；唯一正文权威在 `src/application/roleContract.js`，本段只述语义不复制正文）。(1) **WQ-01 grounding**——对仓库路径/端点/符号/模块/API 形状/测试行为做具体断言前，先检查相关仓库证据，并在最终报告中给出支撑的 repo-relative 路径/符号/测试；未验证的断言必须显式标为 unverified/uncertain，不得当事实陈述。(2) **WQ-02 async/state**——改动 async/有状态 UI 或 query-gating 行为时，枚举适用状态（normal、loading、error、missing、unparseable、stale-data-plus-error），测试每个适用状态及高风险组合；省略某状态须说明为何不适用。该块是 **worker 执行/汇报纪律，不是语义验收规则**：不引入 parser、scorecard、自动重试/自动打回、自动门或接受决策，**Lead 仍是唯一语义裁判**。只对已有非空 role contract 且 agentId 合法的 worker 生效（start/resume 同一 SSOT，恰好一次）；无 role contract 或非法 agentId 行为不变；块随既有 roleContract transport 注入，不新增能力门，也不强制不支持 role 注入的 backend 承载它；六种正式 canonical 身份的组成串除 agentId 令牌外逐字节一致（无 runtime 名称/worker 席位分支）。
...
**M12-13 readiness 投影**：该事件与 `run_delivery` readiness 闭集投影相连——终端 delivery-requested run 且
唯一较高优先级 delivery 事实为恰好一条 run-bound 安全 `isolation_violation` 时，readiness 投影为
`isolation_failed` + `isolationFailure:{code:"workdir_escape"}`（与 packaging failure 严格分离；无
candidateInventory/repackage/salvage/retry/stop/decision 面，Lead 只能另行派发）；facts 优先、
缺失/多条/跨 run/非闭集 code → `ambiguous`（见 §4.6 上方 `runDelivery.js` 条目）。

**M12-14 worker-visible Work Order SSOT 与两道 containment 门的时序**：`composeDeliveryExecutionContract({allowedPaths})`
（`src/application/roleContract.js`）在控制面已备好授权路径列表时，把 AUTHORIZED WORK SCOPE 块附加到既有
delivery execution contract——单行 `AUTHORIZED_PATHS_JSON`（`encodeAuthorizedPathsJson` 转义
`U+2028/U+2029 与 DEL/C1（0x7F–0x9F）` 保证恒为一行，路径无法伪造新合同字段）逐字节声明 worker 开工前必须看到的精确写路径
契约（segment-boundary 大小写敏感的匹配规则 + 越界时 `SCOPE_EXPANSION_REQUIRED` 上报而非编辑的指令）。
worker 开工前看到的是权威清单，但 packaging containment 未放松——块只声明 scope，不替换 packager。
两道 containment 门时序分明、均为机械判定、不裁决语义也不自动 stop：(1) **观察到的**写路径位于 isolated
worktree **之外** → `run.isolation_violation(code=workdir_escape)`，在 packaging **之前**转 failed（M12-13
`isolation_failed`，无 packaging/decision 面）；(2) 位于 worktree **之内**但与 `delivery.allowedPaths`
不匹配或越出 path-segment boundary → packaging **期间**以 `disallowed_path` 拒绝（retained candidate 保留，
`run_delivery_repackage` 无模型重封装）。
...
delivery: {
  ...
  verificationTimeoutMs: 600000       // M12-13 可选：per-command 执行预算（整数 [1000,7200000]）
}
```
...
worktree 创建前用 `prepareDeliveryRequest()` 验证（SSOT 在 `src/delivery.js`）——M12-13 起 `verificationTimeoutMs` 也在此 SSOT 校验（整数 ms，闭界 `VERIFICATION_TIMEOUT_MS_MIN=1000`/`VERIFICATION_TIMEOUT_MS_MAX=7200000` 与 MCP zod 同源；非法值在任何副作用前以 `invalid_verification` 拒绝；**仅在显式声明时持久化**，缺失零漂移）。
worktree 创建后、backend spawn 前捕获 base commit（full hash）。
`run.started` 扩展 `delivery: {mode, baseCommit, allowedPaths, verificationCommands|verificationUnavailableReason, verificationTimeoutMs?}`（`verificationTimeoutMs` 仅当 Lead 声明时出现）。
...
6. **M12-13 per-command 执行预算**（`verificationTimeoutMs`，Lead 可选的执行超时/预算）：Lead 声明时，`prepareDeliveryRequest`（SSOT）校验为闭界整数 ms `[1000, 7200000]`，默认 `300000` **仅在字段缺失时**应用（缺失时持久化零漂移——`run.started.delivery`、`delivery_created` ref、verification outcome ref 都不出现该键）。它是 exact verifier 的**逐条** setup/assertion 命令执行预算（与 `run_wait`/`run_await_result` 的 Lead 观察窗口正交，语义上**不是** observation budget）。持久值权威：start/resume（resume 重新经 SSOT 校验，持久值损坏 → resume 拒绝 null，零副作用）、profile 折叠（profile 只供命令，Lead 声明的预算保留）、MCP dispatch/continue/CLI 转发、reverify 继承（省略 `timeoutMs` 继承 ref 持久预算，持久值损坏 fail-closed；显式值同样过共享闭界）全部保留同一字段。执行超时结果投影为闭集 `command_timeout`/`setup_timeout`；**无自动加宽、无自动重试、无自动 stop/decision**。
