...
// ============================================================
// M11-2 real Lead dogfood docs-only SSOT closeout.
// A fresh Codex Lead completed the WAO control loop on a disposable project
// (verdict PASS_WITH_HOST_FRICTION). These guards keep the roadmap honest:
// the M11-2 dogfood gate is closed, but M11 overall stays in progress, and
// host/plugin frictions (Serena, Codex model-cache) must not be rewritten as
// WAO blockers. The acceptance is a fresh Codex CLI task, NOT a Codex Desktop
// window-launch verification.
// ============================================================

test("M11-2-DOGFOOD-01: roadmap 仍开放列表不得包含 M11-2 dogfood", () => {
  const roadmap = read("docs/roadmap.md");
  // 提取 M11 行（以 | M11 | 开头的表格行）。
  const m11Row = roadmap.split("\n").find((l) => /^\|\s*M11\s*\|/.test(l)) || "";
  assert.ok(m11Row, "roadmap 含 M11 行");
  // 提取"仍开放"到下一个句号（中文。）之间的开放项清单，不覆盖其后"M11 整体仍进行中"
  // 的总结句（那句话会合法地提到 dogfood 已完成）。
  const openMatch = m11Row.match(/仍开放[^。]*。/);
  const openList = openMatch ? openMatch[0] : "";
  // 开放项清单里不得把 M11-2 dogfood 列为待办。
  assert.ok(!/M11-2.*dogfood|dogfood.*M11-2|M11-2.*真实.*Lead|真实.*Lead.*dogfood/i.test(openList),
    "roadmap '仍开放' 清单不得再列 M11-2 真实 Lead dogfood（dogfood 门已关闭）");
});

test("M11-2-DOGFOOD-02: roadmap 标记 M11-2 fresh Codex Lead dogfood 已完成", () => {
  const roadmap = read("docs/roadmap.md");
  const m11Row = roadmap.split("\n").find((l) => /^\|\s*M11\s*\|/.test(l)) || "";
  // 必须在 M11 行中表达 M11-2 dogfood 已完成，并提及 fresh Codex Lead。
  assert.ok(/M11-2.*dogfood.*完成|dogfood.*M11-2.*完成|M11-2.*dogfood.*已交|M11-2.*dogfood.*PASS|M11-2.*dogfood.*通过/i.test(m11Row),
    "roadmap 必须标记 M11-2 dogfood 已完成/通过");
  assert.ok(/fresh Codex|全新 Codex|fresh.*Codex/i.test(m11Row),
    "roadmap 必须说明 dogfood 由 fresh Codex Lead 完成（非 Desktop）");
  // 必须含最小证据锚点之一（runId 或 delivery commit），不要求复制完整审计。
  assert.ok(/run_202607192128556114jk5v4|cc4bfda/i.test(m11Row),
    "roadmap 必须含 runId 或 delivery commit 作为最小证据锚点");
});

test("M11-2-DOGFOOD-03: M11 整体仍标为进行中，不得标整个 M11 完成", () => {
  const roadmap = read("docs/roadmap.md");
  const m11Row = roadmap.split("\n").find((l) => /^\|\s*M11\s*\|/.test(l)) || "";
  assert.ok(/进行中|M11 整体未完成|M11 仍.*开放|M11 整体.*进行中/i.test(m11Row),
    "M11 整体仍标为进行中");
  assert.ok(!/^\|\s*M11\s*\|\s*✅\s*完成\b/.test(m11Row),
    "M11 整体行不得标为 ✅ 完成（M11-2 dogfood 完成不等于整个 M11 完成）");
});

test("M11-2-DOGFOOD-04: 活文档不得声称 Codex Desktop 已验收", () => {
  const roadmap = read("docs/roadmap.md");
  // dogfood 验收是 fresh Codex CLI 任务，不是 Desktop 窗口启动 UX 验收。
  // 允许否定澄清（"不是 Codex Desktop 验收"），禁止肯定声称（"Codex Desktop 已验收/通过"）。
  // 先剥离否定前缀（不是/并非/not），再检测肯定声称。
  const stripped = roadmap.replace(/不是\s*Codex\s*Desktop|并非\s*Codex\s*Desktop|not\s+(?:a\s+)?Codex\s+Desktop|不是.*Desktop/gi, "");
  assert.ok(!/Codex\s*Desktop\s*(已)?\s*(验收|通过|verified|passed|完成)/i.test(stripped),
    "活文档不得声称 Codex Desktop 已验收（dogfood 是 fresh Codex CLI 任务；否定澄清除外）");
});

test("M11-2-DOGFOOD-05: 活文档不得把 Serena friction 写成 WAO blocker", () => {
  const roadmap = read("docs/roadmap.md");
  // Serena 是 host/plugin friction，已由 Owner 禁用，不是 WAO blocker 或新 TD。
  // 允许否定澄清（"非 WAO blocker"），禁止肯定声称（"Serena 是 WAO blocker/阻塞/必须修复"）。
  // 先剥离否定前缀（非/不是/not），再检测肯定声称。
  const stripped = roadmap.replace(/Serena[^。]*?(?:非|不是|not)\s*(?:WAO\s*)?blocker/gi, "");
  assert.ok(!/Serena[^。]*?(?:WAO\s*)?(?:blocker|阻塞|must.?fix|必须修复)|Serena[^。]*?TD-?\d/i.test(stripped),
    "活文档不得把 Serena host/plugin friction 写成 WAO blocker 或新 TD（否定澄清除外）");
});
