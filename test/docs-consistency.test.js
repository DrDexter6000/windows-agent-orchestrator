...
test("M10-pre2: workspace_status tool documented in usage.md and SKILL.md", () => {
  ...
  // + run_delivery_review (M11-3) = 14; + workspace_select (M11-6) = 15.
  assert.ok(/15 MCP tools/.test(skill), "SKILL.md must reflect 15 MCP tools after M11-6");
...
test("M11-3D1: usage.md MCP 段反映当前 15 tools", () => {
  const usage = read("docs/usage.md");
  assert.ok(!/7 个工具/.test(usage), "usage.md MCP 段不得再声称只有 7 个工具");
  assert.ok(/15 个工具/.test(usage), "usage.md MCP 段必须反映 15 个工具（含 M11-6 workspace_select）");
});
...
test("M11-3D1: SKILL/architecture 当前工具事实为 15 tools", () => {
  const skill = read("SKILL.md");
  const arch = read("docs/02-architecture.md");
  assert.ok(/15 MCP tools|15 tools/i.test(skill),
    "SKILL.md Minimal MCP Loop 当前工具数为 15（含 M11-6 workspace_select）");
  // 精确匹配 "server.js ... N tools" 的当前状态注释行。
  const serverLine = arch.split("\n").find((l) => /server\.js.*tools/.test(l)) || "";
  assert.ok(/15 tools/.test(serverLine),
    "architecture server.js 注释当前工具数为 15");
});
...
test("M11-2C-12: SKILL tool-count 文案不得声称 minimal loop 必须经过全部工具", () => {
  ...
  // 必须表达：WAO 暴露 15 tools，但 minimal control loop 只用相关 control tools，
  // playbook reads 是可选且在 dispatch loop 外。
  assert.ok(/15 MCP tools|15 tools/i.test(skill),
    "SKILL 声明 WAO 暴露 15 MCP tools");
