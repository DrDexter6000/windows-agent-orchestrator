...
test("M10-pre2: workspace_status tool documented in usage.md and SKILL.md", () => {
  ...
  // + run_delivery_review (M11-3) = 14.
  assert.ok(/14 MCP tools/.test(skill), "SKILL.md must reflect 14 MCP tools after M11-3");
...
test("M11-3D1: usage.md MCP 段反映当前 14 tools", () => {
  const usage = read("docs/usage.md");
  assert.ok(!/7 个工具/.test(usage), "usage.md MCP 段不得再声称只有 7 个工具");
  assert.ok(/14 个工具/.test(usage), "usage.md MCP 段必须反映 14 个工具");
});
...
test("M11-3D1: SKILL/architecture 当前工具事实为 14 tools", () => {
  const skill = read("SKILL.md");
  const arch = read("docs/02-architecture.md");
  assert.ok(/14 MCP tools|14 tools/i.test(skill),
    "SKILL.md Minimal MCP Loop 当前工具数为 14");
  // 精确匹配 "server.js ... N tools" 的当前状态注释行。
  const serverLine = arch.split("\n").find((l) => /server\.js.*tools/.test(l)) || "";
  assert.ok(/14 tools/.test(serverLine),
    "architecture server.js 注释当前工具数为 14");
});
...
test("M11-2C-12: SKILL tool-count 文案不得声称 minimal loop 必须经过全部工具", () => {
  ...
  const mandatoryAll = /full\s+minimal\s+loop\s+through\s+14|minimal\s+loop\s+必须.*全部\s*14|loop\s+must\s+(use|go through|include)\s+all\s+14/i;
  assert.ok(!mandatoryAll.test(skill),
    "SKILL tool-count 文案不得声称 minimal loop 必须经过全部工具");
  // 必须表达：WAO 暴露 14 tools，但 minimal control loop 只用相关 control tools，
  // playbook reads 是可选且在 dispatch loop 外。
  assert.ok(/14 MCP tools|14 tools/i.test(skill),
    "SKILL 声明 WAO 暴露 14 MCP tools");
